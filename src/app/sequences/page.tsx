...
export default function SequencesPage() {
  ...
  const createCampaign = async () => {
    ...
    const res = await fetch("/api/campaigns", {
      ...
      body: JSON.stringify({ name, dailyLimit: 25 }),
    });
    const campaign = await res.json();
    const normalized = { ...campaign, totalContacts: 0 };
  ...
  /* =========================
     DELETE CAMPAIGN
  ========================= */
  const handleDeleteCampaign = (deletedId: string) => {
    setCampaigns((prev) => prev.filter((c) => c.id !== deletedId));
    // If the deleted campaign was selected, select the next one
  ...
  return (
    <AppShell activeTab="campaigns">
      <div className="flex flex-1 overflow-hidden">
        {/* LEFT PANEL */}
        <div className="w-[300px] border-r border-[#ececf4] bg-[#fcfcff] flex flex-col flex-shrink-0 overflow-y-auto">
          <div className="p-5 border-b border-[#ececf4] flex items-center justify-between">
            <h2 className="text-lg font-bold text-gray-900">Campaigns</h2>
            <button
              onClick={createCampaign}
              className="w-10 h-10 rounded-2xl bg-gradient-to-r from-violet-500 to-fuchsia-500 flex items-center justify-center text-white shadow-md hover:scale-105 transition-all duration-200"
            >
              <Plus size={14} />
            </button>
          </div>
