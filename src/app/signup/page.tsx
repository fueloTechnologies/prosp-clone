...
export default function SignupPage() {
  ...
  const passwordStrength = !password ? 0 : password.length < 4 ? 1 : password.length < 7 ? 2 : password.length < 10 ? 3 : 4;
  const strengthLabel = ["", "Too short", "Weak", "Good", "Strong"][passwordStrength];
  const strengthColor = ["", "#ef4444", "#eab308", "#3b82f6", "#22c55e"][passwordStrength];
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) { setError(data.error || "Signup failed. Please try again."); return; }
      router.push("/login");
    } catch {
  ...
  const inputBox = (focused: boolean) => ({
    ...
    borderRadius: 14, border: `1px solid ${focused ? "rgba(59,130,246,0.5)" : "rgba(255,255,255,0.1)"}`,
    background: "rgba(255,255,255,0.03)", padding: "0 16px", height: 50,
  });

  return (
    <div style={{ position: "fixed", inset: 0, overflowY: "auto", background: "#050816", color: "white", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      {/* Background glows */}
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        ...
        <div style={{ position: "absolute", top: "20%", left: "50%", transform: "translateX(-50%)", width: 700, height: 400, background: "rgba(59,130,246,0.05)", filter: "blur(120px)", borderRadius: "50%" }} />
        ...
        <div style={{ width: "100%", maxWidth: 440 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 40 }}>
            ...
            <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ color: "#6b7280", fontSize: 13 }}>AI Powered LinkedIn Outreach</span>
            </div>
          </div>
          ...
          <div style={{ borderRadius: 32, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(7,13,31,0.7)", backdropFilter: "blur(32px)", padding: 36, boxShadow: "0 0 100px rgba(59,130,246,0.08), inset 0 1px 0 rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ marginBottom: 28 }}>
              <h2 style={{ fontSize: 24, fontWeight: 800, color: "white", margin: 0, letterSpacing: -0.3 }}>Create your account</h2>
              <p style={{ color: "#6b7280", fontSize: 14, margin: "6px 0 0" }}>Start automating your LinkedIn outreach today.</p>
            </div>
            ...
            <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24 }}>
              ...
              <span style={{ position: "relative", background: "#070d1f", padding: "0 14px", fontSize: 11, color: "#4b5563", letterSpacing: "0.15em", textTransform: "uppercase" }}>or sign up with email</span>
            </div>
            ...
            <form onSubmit={handleSignup} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {/* Name */}
              <div>
                <label style={{ display: "block", fontSize: 12, color: "#9ca3af", marginBottom: 8, letterSpacing: "0.05em" }}>Full Name</label>
                <div style={{ display: "flex", alignItems: "center", gap: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)", padding: "0 16px", height: 50 }}
              ...
              {/* Password */}
              <div>
                <label style={{ display: "block", fontSize: 12, color: "#9ca3af", marginBottom: 8, letterSpacing: "0.05em" }}>Password</label>
                <div style={{ display: "flex", alignItems: "center", gap: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)", padding: "0 16px", height: 50 }}
                ...
                {password && (
                  <div style={{ marginTop: 8 }}>
                    <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                      {[1,2,3,4].map(level => (
                        <div key={level} style={{ flex: 1, height: 3, borderRadius: 4, background: level <= passwordStrength ? strengthColor : "rgba(255,255,255,0.1)", transition: "background 0.3s" }} />
                      ))}
                    </div>
              ...
              {/* Confirm Password */}
              <div>
                <label style={{ display: "block", fontSize: 12, color: "#9ca3af", marginBottom: 8, letterSpacing: "0.05em" }}>Confirm Password</label>
                <div style={{ display: "flex", alignItems: "center", gap: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)", padding: "0 16px", height: 50 }}
                  onFocusCapture={e => (e.currentTarget.style.borderColor = "rgba(139,92,246,0.5)")}
                  onBlurCapture={e => (e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)")}
                >
                  <Lock size={16} color="#6b7280" />
                  <input type={showConfirm ? "text" : "password"} placeholder="Repeat password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
                    style={{ background: "transparent", border: "none", outline: "none", width: "100%", color: "white", fontSize: 14 }} />
                  <button type="button" onClick={() => setShowConfirm(!showConfirm)} style={{ background: "none", border: "none", cursor: "pointer", color: "#6b7280", display: "flex", padding: 0 }}>
              ...
              <button
                type="submit"
                disabled={loading}
                style={{ width: "100%", height: 50, borderRadius: 14, background: "linear-gradient(to right,#2563eb,#7c3aed)", border: "none", color: "white", fontWeight: 700, fontSize: 16, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 8px 32px rgba(37,99,235,0.3)", transition: "all 0.2s", opacity: loading ? 0.8 : 1 }}
