    ...
    return (
      <div className="bg-white rounded-2xl border border-border overflow-hidden p-4 space-y-2">
        {[1, 2, 3, 4, 5].map((i) => (
          <div
            key={i}
            className="h-12 bg-surface-secondary rounded-xl animate-pulse"
          />
        ))}
      </div>
    );
                          ...
                          <div className="absolute right-0 top-8 bg-white border border-border rounded-xl shadow-modal w-36 py-1 z-20">
                            {c.linkedInUrl && (
                              <a
                                ...
                                className="w-full flex items-center gap-2 px-3 py-2 text-sm text-ink hover:bg-surface-secondary transition-colors"
                              >
